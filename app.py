...
# ==========================================
# [탭 2] 글로벌 이메일 자동 발송기 (제목 수정 및 템플릿 저장 완비)
# ==========================================
with tab2:
    ...
    # --- 1. 영구 템플릿 저장소 (제목과 내용을 세트로 관리) ---
    # 이 부분의 텍스트를 수정해 두시면 깃허브에 영구적으로 저장되어 언제든 꺼내 쓸 수 있습니다.
...
<p>เรากำลังมองหาพาร์ทเนอร์ด้านการตลาดเพื่อขยายตลาดในประเทศของคุณ...</p>"""
                },
                "English": {
                    "subject": "[Marketing Collaboration] Premium K-Beauty Brand zenifix",
                    "body": "<p>Dear Beauty Marketing Team,</p>\n<p>We are looking for a marketing partner...</p>"
                }
            }
        }
    ...
    with st.expander("➕ 새로운 타깃 그룹 및 언어 템플릿 추가하기"):
        st.info("자주 쓰는 새로운 대상과 언어를 추가합니다. (※ 영구 저장을 원하시면 코드 상단의 저장소 딕셔너리에 직접 추가해 주세요.)")
    ...
    # --- 3. 이메일 미리보기 및 자유 편집기 (제목 추가) ---
    st.subheader("📝 이메일 미리보기 및 직접 편집")
    
    # 선택된 템플릿의 제목과 본문 불러오기
...
"""
    ...
    st.markdown("##### 👁️ 실제 이메일 수신자가 받아볼 이메일 미리보기")
    ...
    with col_btn2:
        if st.button("🚀 전체 엑셀 리스트 대량 발송 시작", type="primary", use_container_width=True):
            if not uploaded_file or not login_email or not app_password:
                ...
                st.info(f"총 {len(df)}명의 수신자에게 발송을 시작합니다...")
                ...
                try:
                    ...
                    server.quit()
                    st.success(f"🎉 총 {success_count}건 발송 및 DB 저장이 완료되었습니다!")
